                ...
                <div className="flex items-center gap-2 shrink-0">
                  {c.pipeline_state === "AWAITING_APPROVAL" && (
                    <>
                      ...
                      <button onClick={() => handleApprove(c)} className="btn-success text-xs px-3 py-1.5">
                        Quick Approve
                      </button>
                    </>
                  )}
    ...
    <div className="view-enter max-w-3xl space-y-8">
      ...
      <div className="card-brand p-8 rounded-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-blue-100 mt-2 text-sm leading-relaxed max-w-lg">
            Describe your campaign in plain English — our multi-agent AI extracts targets, crafts content, and orchestrates outreach at global scale.
          </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="card p-6 space-y-4">
          <div className="flex items-center justify-between">
            <label className="block text-sm font-semibold text-slate-800">
              ...
              <span className="ml-2 text-xs font-normal text-slate-400">
                More detail = better AI performance
              </span>
            </label>
            <button
              type="button"
          ...
          <div className="flex items-center justify-between">
            <p className="text-xs text-slate-400 italic">
              Tip: mention company, audience, location, channel &amp; tone
            </p>
        ...
        <div className="card p-5 flex items-center gap-4">
          ...
          <div>
            <div className="text-sm font-semibold text-slate-800">Auto-approve Content</div>
            <div className="text-xs text-slate-400 mt-0.5">
              {autoApprove
                ? "AI content will be dispatched immediately without review"
                : "You will review AI-generated content per contact before sending"}
            </div>
          </div>
        </div>
...
function DetailView({
  ...
  function renderCommonPreview(channel: string) {
    ...
    const renderField = (v: string) => v.replace(/\{(\w+)\}/g, (_, k) => contact[k] ?? `[${k}]`);
                    ...
                    co && (
                      <div className="bg-slate-50 rounded-lg p-3 text-xs text-slate-600 space-y-1 border border-slate-100">
                        {Object.entries(co).map(([k, v]) =>
                          typeof v === "string" ? (
                            <div key={k}><span className="text-slate-400 mr-1">{k}:</span>{v}</div>
                          ) : null,
                        )}
                      </div>
                    )
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        {campaign.pipeline_state === "AWAITING_APPROVAL" && (
          <>
            ...
            <button onClick={() => onApproval(campaign)} className="btn-warning text-sm px-5 py-2.5">
              ✎ Review via WebSocket
            </button>
          </>
        )}
        <button onClick={() => onAnalytics(campaign)} className="btn-brand text-sm px-5 py-2.5">
          ◎ Analytics
        </button>
      </div>
    </div>
...
function ApprovalView({
  ...
  function connect() {
    ...
    addLog(`[WS] Connecting to campaign ${campaign.id.slice(0, 8)}…`);
    const ws = openApprovalWs(
      ...
      (msg) => {
        ...
        if (msg.type === "APPROVAL_START") {
          ...
          toast(`Approval session started — ${msg.total_contacts} contacts`, "info");
        }
        if (msg.type === "CHANNEL_GROUP_START") {
          setCurrentChannel(msg.channel ?? "");
          toast(`Next group: ${msg.channel} (${msg.count} contacts)`, "info");
        }
        ...
        if (msg.type === "CAMPAIGN_APPROVED") {
          setDone(true);
          toast(`Campaign fully approved by ${msg.approved_by}!`, "success");
          onDone();
        }
        if (msg.error) {
  ...
  function sendApprove()    { send({ action: "approve" }); }
  function sendApproveAll() { send({ action: "approve_all" }); }
  function sendRegenerate() { send({ action: "regenerate" }); }
  function sendEdit()       { send({ action: "edit", edited_content: editFields as Record<string, unknown> }); }

  async function restApprove() {
    const { error } = await approveCampaign(campaign.id);
    if (error) { toast(error, "error"); return; }
    toast("Campaign approved via REST!", "success");
    onDone();
  }

  function renderContentEditor() {
    if (!currentContact) return null;
    const ch = currentContact.channel ?? "Email";
    ...
    const fields = ch === "Email" ? EMAIL_FIELDS : ch === "LinkedIn" ? LINKEDIN_FIELDS : CALL_FIELDS;

    return (
      <div className="space-y-3">
        {fields.map((field) => (
          <div key={field}>
            <label className="block text-[11px] text-slate-400 uppercase tracking-wider mb-1 capitalize">
              {field.replace(/_/g, " ")}
            </label>
            <textarea
              value={editFields[field] ?? ""}
              onChange={(e) => setEditFields((prev) => ({ ...prev, [field]: e.target.value }))}
              rows={field === "body" || field === "message" || field === "value_prop" ? 4 : 2}
              className="input-premium text-xs resize-y"
            />
          </div>
        ))}
  ...
  return (
    <div className="view-enter space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-900 font-display">Approval Review</h2>
          <p className="text-slate-500 text-sm truncate">{campaign.name}</p>
        </div>
      ...
      {Object.keys(channelCounts).length > 0 && (
        <div className="flex gap-3 flex-wrap">
          {Object.entries(channelCounts).map(([ch, count]) => (
            <div key={ch} className="card-flat px-4 py-2 flex items-center gap-2">
      ...
      {done ? (
        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-8 text-center">
          <div className="text-4xl mb-3">✓</div>
          <div className="text-emerald-700 font-semibold text-lg font-display">
            All {totalContacts} contacts approved!
          </div>
          <div className="text-slate-500 text-sm mt-1">Campaign is being dispatched.</div>
          <button onClick={onDone} className="mt-5 btn-brand px-6 py-2.5">
            Back to Dashboard
          </button>
        </div>
      ) : (
        <div className="grid lg:grid-cols-2 gap-5">
          <div className="space-y-4">
            <div className="card p-5">
              <div className="flex justify-between text-sm text-slate-500 mb-3">
                <span>Progress</span>
                <span className="font-medium text-slate-700">{approved} / {totalContacts || "?"} approved</span>
              </div>
              <div className="h-2.5 bg-slate-100 rounded-full overflow-hidden">
                <div
                  className="h-full bg-gradient-to-r from-blue-500 to-blue-600 bar-fill progress-bar-animated rounded-full"
                  style={{ width: totalContacts ? `${pct(approved, totalContacts)}%` : "0%" }}
                />
              </div>
              {currentChannel && (
                <div className="mt-3 text-xs text-blue-600">
                  {CHANNEL_ICONS[currentChannel] ?? "📨"} Reviewing:{" "}
                  <span className="font-semibold">{currentChannel}</span> group
                </div>
              )}
            </div>
            ...
            <div className="card p-5 space-y-3">
              ...
              {connected && (
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className="btn-success text-xs py-2 disabled:opacity-40"
                  >
                    ✓ Approve This
                  </button>
                  <button onClick={sendApproveAll} className="btn-success text-xs py-2">
                    ✓✓ Approve All
                  </button>
                  <button
                    onClick={sendRegenerate}
                    disabled={!currentContact}
                    className="btn-warning text-xs py-2 disabled:opacity-40"
                  >
                    ↺ Regenerate
                  </button>
                  <button
              ...
              <div className="border-t border-slate-100 pt-3">
                <p className="text-xs text-slate-400 mb-2">Or approve via REST API:</p>
                <button
                  onClick={restApprove}
            ...
            {currentContact && (
              <div className="card p-5 border-amber-100">
                <div className="flex items-center gap-2 mb-4">
                  <span className="text-sm font-medium text-slate-800">{currentContact.contact_email}</span>
          ...
          {/* WS log */}
          <div className="card-flat bg-slate-900 rounded-2xl p-4">
            <div className="text-xs text-slate-500 uppercase tracking-wider mb-3">WebSocket Log</div>
            <div
              ref={logRef}
              className="ws-log text-slate-400 h-[480px] overflow-y-auto space-y-0.5"
            >
              {log.length === 0 ? (
                <div className="text-slate-600 italic">Click "Connect WebSocket" to start…</div>
              ) : (
                log.map((line, i) => (
                  <div
                    key={i}
                    ...
                    <div className="divide-y divide-slate-50 max-h-[420px] overflow-y-auto">
                      {logs.map((log) => (
                        <div key={log.id} className="px-5 py-3 flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="text-xs text-slate-500 mb-0.5">{fmt(log.timestamp)}</div>
                            <div className="text-sm text-slate-700">{log.step}</div>
                            {log.message && <div className="text-xs text-slate-400 mt-0.5">{log.message}</div>}
...
function SettingsView({
  ...
  async function handleRefresh() {
    ...
    if (error || !data) { toast(error ?? "Failed", "error"); return; }
    setTokens(data.access_token, data.refresh_token);
    setRefreshResult(`New access_token: ${data.access_token.slice(0, 40)}…`);
    toast("Token refreshed!", "success");
  }

  async function testEndpoints() {
    toast("Testing server connectivity…", "info");
    try {
      const r = await fetch("http://localhost:8000/openapi.json");
      if (r.ok) toast("✓ Server reachable — OpenAPI schema loaded", "success");
      else toast(`Server returned HTTP ${r.status}`, "error");
    } catch {
      toast("Cannot reach http://localhost:8000 — is the server running?", "error");
    }
  }
  ...
  return (
    <div className="view-enter space-y-6 max-w-lg">
      <div>
        ...
        <p className="text-slate-500 text-sm">Account, tokens, and diagnostics</p>
      </div>
      ...
      <div className="card p-6 space-y-4">
        ...
        <div className="space-y-3">
          <div>
            <div className="text-xs text-slate-400 mb-1 font-medium uppercase tracking-wider">Email</div>
            <div className="text-sm text-slate-700 font-medium">{userEmail}</div>
          </div>
          <div>
            <div className="text-xs text-slate-400 mb-1 font-medium uppercase tracking-wider">Role</div>
            <div
          ...
          <div>
            <div className="text-xs text-slate-400 mb-1 font-medium uppercase tracking-wider">API Base</div>
            <div className="text-xs text-slate-500 font-mono bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100">
      ...
      <div className="card p-6 space-y-4">
        <h3 className="text-sm font-semibold text-slate-800">Manual Token Refresh</h3>
        <div>
          <label className="block text-xs text-slate-400 mb-1.5 font-medium">Refresh Token</label>
          <input
            value={refreshToken}
        ...
        <button
          ...
          className="btn-brand text-sm px-5 py-2 disabled:opacity-40"
        >
          Refresh Token (POST /auth/refresh)
        </button>
        {refreshResult && (
          <pre className="text-xs text-emerald-700 bg-emerald-50 rounded-lg p-3 break-all whitespace-pre-wrap border border-emerald-100">
            {refreshResult}
          </pre>
        )}
      </div>
      ...
      <div className="card p-6 space-y-4">
        <h3 className="text-sm font-semibold text-slate-800">Diagnostics</h3>
        <button
        ...
        <div className="text-xs text-slate-400 space-y-1.5">
          <p>API: <span className="text-slate-600 font-mono bg-slate-50 px-2 py-0.5 rounded">http://localhost:8000/api/v1</span></p>
          <p>WebSocket: <span className="text-slate-600 font-mono bg-slate-50 px-2 py-0.5 rounded">ws://localhost:8000/api/v1</span></p>
          <p>Docs: <a href="http://localhost:8000/docs" target="_blank" rel="noreferrer" className="text-blue-500 underline">localhost:8000/docs</a></p>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  function renderMain() {
    switch (view) {
      ...
      case "dashboard":
        return (
          <DashboardView
            ...
            onSelect={(c) => { setSelectedCamp(c); setView("detail"); }}
      ...
      case "history":
        return <HistoryView campaigns={campaigns} toast={toast} />;
      case "settings":
        return (
          <SettingsView
            userEmail={userEmail}
